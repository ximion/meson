...
''')
    ...
    def use_cross_list(self, kwargs):
        if 'native' in kwargs:
            ...
            return self.environment.is_cross()

    @permittedKwargs(permitted_kwargs['add_global_arguments'])
    @stringArgs
    def func_add_global_arguments(self, node, args, kwargs):
        arg_obj = self.build.global_cross_args if self.user_cross_list(kwargs) else self.build.global_args
        self.add_global_arguments(node, arg_obj, args, kwargs)

    @permittedKwargs(permitted_kwargs['add_global_link_arguments'])
    @stringArgs
    def func_add_global_link_arguments(self, node, args, kwargs):
        arg_obj = self.build.global_cross_link_args if self.user_cross_list(kwargs) else self.build.global_link_args
        self.add_global_arguments(node, arg_obj, args, kwargs)

    @permittedKwargs(permitted_kwargs['add_project_arguments'])
    @stringArgs
    def func_add_project_arguments(self, node, args, kwargs):
        arg_obj = self.build.projects_cross_args if self.user_cross_list(kwargs) else self.build.projects_args
    ...
    def func_add_project_link_arguments(self, node, args, kwargs):
        arg_obj = self.build.projects_cross_link_args if self.user_cross_list(kwargs) else self.build.projects_link_args
        self.add_project_arguments(node, arg_obj, args, kwargs)
